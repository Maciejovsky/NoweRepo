...
def recommendations(request):
    obj = Meals.objects.filter(score=0).order_by("?").first()
    contex = {
        'object':obj
    }
    return render(request, 'orders/recommendations.html',contex)
    #if request.user.is_authenticated:
     #   return render(request, "orders/recommendations.html", context={"meals": Meals.objects.filter(score = 0).order_by("?").first()})
    #else:
     #   return redirect("orders:login")
...
def rate_image (request):
    if request.method == "POST":
        ...
        obj = Meals.objects.get(id=el_id)
        obj.score = val
        obj.save()
